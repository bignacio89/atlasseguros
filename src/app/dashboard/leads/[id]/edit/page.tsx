  ...
  return (
    <div className="space-y-6">
      <section className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 mb-2">Editar lead</h1>
          <p className="text-sm text-slate-600">
            Actualiza datos del lead y registra nuevos consentimientos GDPR.
          </p>
        </div>
      ...
      <form action={updateLeadAction} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-6">
        <input type="hidden" name="id" value={lead.id} />
        <input type="hidden" name="privacyPolicyVersion" value="v1" />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Nombre">
            <input name="firstName" defaultValue={lead.firstName} required className={inputClass} />
          </Field>
          <Field label="Apellidos">
            <input name="lastName" defaultValue={lead.lastName} required className={inputClass} />
          </Field>
          <Field label="Email">
          ...
          <Field label="Tipo de lead">
            <select name="leadType" defaultValue={lead.leadType} className={inputClass}>
